...
class WpMediaFolder extends WpmfHelper
{
     ...
     * @return void
     */
    public function mediaSetFilesToFolderWpml($attachment_id)
    {
    ...
    {
        $wcat    = isset($_GET['wcat']) ? $_GET['wcat'] : '0'; // phpcs:ignore WordPress.CSRF.NonceVerification.NoNonceVerification -- No action, nonce is not required
        $parents = array();
        $pCat    = (int) $wcat;
        while ((int) $pCat !== 0) {
            $parents[] = $pCat;
     ...
     *
     * @param integer $attachment_id Id of attachment
     * @param integer $parent        Parent of attachment
     *
     * @return void
     */
            ...
            if ($trid) {
                ...
                foreach ($translations as $translation) {
                    if ($translation->element_id !== $attachment_id) {
                         ...
                         * @ignore Hook already documented
                         */
                        do_action('wpmf_attachment_set_folder', $translation->element_id, $parent, array('trigger'=>'upload'));
                    }
                }
            }
        }
    ...
    public function deleteFolder()
    {
        ...
        if ((int) $option_media_remove === 1) {
            ...
            $childs = get_term_children((int) $_POST['id'], WPMF_TAXO);
            $object_in_term = get_objects_in_term($_POST['id'], WPMF_TAXO);
            if ((is_array($childs) && count($childs) > 0) || (is_array($object_in_term) && count($object_in_term)) > 0) {
                // Folder not empty
