...
class TaxonomyField extends Field {
    ...
    public function start() {
        if (is_admin()) {
            ...
            add_action($this->taxonomy . '_edit_form_fields', array(
                $this,
                'display_edit'
            ), 10, 2);
            ...
            add_action('create_' . $this->taxonomy, array(
                $this,
                'update'
            ), 10, 2);
        }
    ...
    public function update($term_id) {
        if (isset($_POST['term_meta'])) {
            ...
            # Overwrite database row with what's in POST.
            foreach ($cat_keys as $key) {
                $term_meta[$key] = $_POST['term_meta'][$key];
            }

            # Checkbox that's unchecked?
            # (If a checkbox is left unchecked, it will not get sent with the POST data)
            if ($this->type === "checkbox" && !isset($_POST['term_meta'][$this->slug])) {
                $term_meta[$this->slug] = "off";
            }
