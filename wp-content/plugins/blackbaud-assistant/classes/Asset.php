...
class Asset extends Core {
    protected $defaults = array("access" => "global", "dependencies" => array(), "version" => "1.0", "in_footer" => true);
    protected $type;
    protected $access;
    ...
    public function add_html() {
        $html = call_user_func_array($this->output, array(
            $this->plugin,
    ...
    public function look_for_shortcode($posts) {
        ...
        foreach ($posts as $post) {
            if (has_shortcode($post->post_content, $this->for_shortcode)) {
