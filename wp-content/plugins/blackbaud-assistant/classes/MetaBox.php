...
class MetaBox extends Core {
    ...
    protected $defaults = array("context" => "advanced", "priority" => "default", 'slug' => 'settings');

    public function create($post) {
        add_meta_box($this->post_type . "_" . $this->slug, __($this->label, $this->post_type), array(
            $this,
            "display"
        ), $this->post_type, $this->context, $this->priority);
    }
    ...
    public function display($post) {
        ...
        if (count($this->field_args)) {
            foreach ($this->field_args as $this_field) {
                ...
                $this->plugin->forge("meta_field", $this_field);
                $field          = $this->plugin->last_forged;
                $fields_html[]  = array(
        ...
        # Print the HTML.
        echo $this->plugin->blackbaud->plugin->get_template("meta-box.blackbaud-assistant.php", array(
            "fields" => $fields_html
        ));
    }
    ...
    protected function start($settings = array()) {
        ...
        if (is_admin()) {
            ...
            add_action("save_post_" . $this->post_type, array(
                $this,
                "save"
            ));
        }
